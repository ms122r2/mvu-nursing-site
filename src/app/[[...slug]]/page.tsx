...
async function readManifest(): Promise<Manifest> {
  const raw = await fs.readFile(path.join(process.cwd(), "_data/manifest.json"), "utf-8");
...
export async function generateMetadata({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  try {
    const manifest = await readManifest();
    const entry = manifest.pages.find((p) => p.path === urlPath || (urlPath === "/" && p.path === "/"));
    if (!entry) return {};
    const data = await readPageData(entry.file);
    const rootProps = (data.root as Record<string, unknown>)?.props as Record<string, string> | undefined;
...
export default async function Page({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  try { manifest = await readManifest(); } catch { return notFound(); }
  const entry = manifest.pages.find((p) => p.path === urlPath || (urlPath === "/" && p.path === "/"));
  if (!entry) return notFound();
  let data: Data;
  try { data = await readPageData(entry.file); } catch { return notFound(); }
